...
const AllTests = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        if (currentUser) {
          const commentsCollection = collection(db, `users/${currentUser}/tests`);
          ...
          <List>
            {comments.map((comment) => (
              <ListItem key={comment.id}>
                <ListItemText primary={comment.text} secondary={` ${comment.date} : ${parseFloat(comment.result.toFixed(1))} % `} />
                <IconButton
                  edge="end"
                  aria-label="delete"
