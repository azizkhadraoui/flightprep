...
function QuestionsFetch() {
    ...
    const handleDeleteQuestion = async (questionId) => {
        try {
            const response = await axios.delete(`http://localhost:8800/api/remove/${questionId}`);
        ...
        <div className="col-md-12 admin">
            <div className="panel">
                <div className="top-bar">
                            <Box component="span" sx={{ background: 'rgba(159, 162, 216, 0.49)' }}>
                ...
                <div className="panel-heading">
                    <div className="row">
                        <div className="col col-sm-3 col-xs-12">
                            <Link to='/create' className='btn btn-secondary mb-3'>Add Question +</Link>
                        </div>
                            ...
                            {records.map(question => (
                                <tr key={question.id}>
                                    ...
                                    <td className='d-flex justify-content-center align-items-center'>
                                        <button className="btn btn-secondary btn-sm mr-2" onClick={() => handleDeleteQuestion(question.id)}>
                                            <i className="material-icons">&#xE872;</i>
                                        </button>
                                        <Link to={`/update/${question.id}`} className="btn btn-secondary btn-sm">
                                            <i className="material-icons">&#xE254;</i>
                                        </Link>
                                    </td>
                                </tr>
                    ...
                    <div className="row">
                        <div className="col-sm-6 col-xs-6">
                            <ul className="pagination hidden-xs pull-right">
                                <li><a href='#' className='page-link' onClick={prePage}> &lt; </a></li>
                                {generatePageNumbers().map((n, i) => (
                                    <li className={`page-item ${currentPage === n ? 'active' : ''}`} key={i}>
                                        <a href='#' className='page-link' onClick={() => changeCPage(n)}>{n}</a>
                                    </li>
                                ))}
                                <li><a href='#' className='page-link' onClick={nextPage}>&gt;</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
